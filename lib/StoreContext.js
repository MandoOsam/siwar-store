...
import { SEED_PRODUCTS, ADMIN_PIN, normalizeProduct } from './constants';
...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('siwar_order_note');
    if (saved) setOrderNoteState(saved);
  }, []);

  const setOrderNote = useCallback((val) => {
    setOrderNoteState(val);
    localStorage.setItem('siwar_order_note', val);
  }, []);

  const clearOrderNote = useCallback(() => {
    setOrderNoteState('');
    localStorage.removeItem('siwar_order_note');
  }, []);
  ...
  return (
    <StoreContext.Provider
      value={{
        ...
        orderNote, setOrderNote, clearOrderNote,
        loaded,
      }}
    >
      {children}
