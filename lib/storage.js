...
export async function insertOrder(row) {
  try {
    ...
    if (error) {
      console.error(`[insertOrder] code=${error.code} message=${error.message} details=${error.details} hint=${error.hint}`);
      return { ok: false, reason: error.message };
    }
    return { ok: true };
  } catch (e) {
    console.error('[insertOrder] exception:', e.message || e);
    return { ok: false, reason: e.message || 'unknown' };
  }
}
...
export async function updateOrderStatus(id, status) {
  ...
  try {
    ...
    if (error) {
      console.error(`[updateOrderStatus] code=${error.code} message=${error.message} details=${error.details} hint=${error.hint}`);
      return { ok: false, reason: error.message };
    }
    return { ok: true };
  } catch (e) {
